...
def caller_path(stackframe):
    return Path(str(locate.locate._file_path_from_stack_frame(stackframe)).replace("/", "\\")).resolve()

def caller_id(stackframe):
    return strhash(
        str(caller_path(stackframe))+str(stackframe.f_lineno)
    )
...
class Source:
    ...
    def __post_init__(self):
        ...
        self.caller_path = caller_path(inspect.stack()[2].frame)
        self.caller_id = caller_id(inspect.stack()[2].frame)
...
class Config:
    def __init__(self, *sources):
        # noinspection PyProtectedMember
        self.caller_path = caller_path(inspect.stack()[1].frame)
        self.caller_id = caller_id(inspect.stack()[1].frame)
