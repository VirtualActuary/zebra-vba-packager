...
with locate.prepend_sys_path(".."):
    from zebra_vba_packager.bas_combining import compile_code_into_sections, compile_bas_sources_into_single_file
...
class TestBasCombining(unittest.TestCase):
    def test_matching(self):
        ...
        self.assertEqual(
            list(match_tokens(tokenize(x), "[private|public] property|sub|function", on_line_start=True)),
            [(1, 4)]
        )
    ...
    def test_compile_code_into_sections_functions(self):
        ...
        self.assertEqual(
            [i.type for i in y],
            ['attribute', 'option', 'unknown', 'function', 'unknown', 'function', 'unknown']
        )
    ...
    def test_compile_code_into_sections_ptrsafe(self):
        ...
        self.assertEqual(
            ['attribute', 'unknown', 'option', 'option', 'unknown', 'declare', 'unknown'],
            [i.type for i in y],
        )
    ...
    def test_combine_bas_sources_into_single_file(self):
        ...
        with self.assertRaisesRegex(ValueError, "Options must be equal across aggregated bas files, got conflict.*"):
